...
@click.option("--corpus_path", type=click.Path(exists=True), help="Path to the corpus. Must be parquet file.",
			  required=True, default=os.path.join(root_dir, "chunked_corpus", "6.parquet"))
@click.option("--raw_path", type=click.Path(exists=True), help="Path to the raw data. Must be parquet file.",
			  required=True, default=os.path.join(root_dir, "parsed_txt", "parsed_result.parquet"))
@click.option("--qa_size", type=int, help="Number of QA pairs to generate.", default=50)
